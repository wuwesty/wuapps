TODOLIST = (function() {
    ...
    function populateCategoryTaskList() {
        function pad(n) {
            return n<10 ? '0'+n : n;
        }
        ...
        for (var ii = 0; ii < currentTasks.length; ii++) {
			var background = "";
			if (currentTasks[ii].category != previousCategory) {
				if (previousCategory != '') listHtml += "</ul>";
				for (var jj = 0; jj < currentCategories.length; jj++) {
					if (currentCategories[jj].name == currentTasks[ii].category) {
						background = currentCategories[jj].color;
					}
				}
				listHtml += "<div class='task-category' id='task-category_" + currentTasks[ii].category + "' style='background: " + background + "'>" + currentTasks[ii].category + "</div>";
				previousCategory = currentTasks[ii].category;
				listHtml += "<ul class='tasklist'>";
			}
            var dueDateHtml = 
                "<ul class='calendar right'>" + 
                    "<li class='day'>" + pad(currentTasks[ii].due.getDate()) + "</li>" +
        ...
        listHtml += "</ul>";
        jQuery("div#categorytasklist").html(listHtml);
    } // populateCategoryTaskList
    ...
    var module = {
        ...
        Storage: (function() {
            ...
            try {
				...
				try {
					...
					db.changeVersion("1.1", "1.2", function(transaction) {
						...
						transaction.executeSql(
							"ALTER TABLE task ADD category TEXT;"
						);
						transaction.executeSql(
					...
					db.changeVersion("1.0", "1.1", function(transaction) {
						transaction.executeSql(
							"ALTER TABLE task ADD completed DATETIME;"
						);
					});
					...
					transaction.executeSql(
						...
						function (transaction, results) {
							...
							for (var ii = 0; ii < results.rows.length; ii++) {
								...
								alert("task");
								$.post("/task", {_doc:taskJson,_docId:task.id}, function() {
            ...
            var subModule = {
                ...
                getTasksInCategoryOrder: function(callback) {
                    subModule.getIncompleteTasks(function(tasks) {
                        callback(tasks.sort(function(taskA, taskB) {
                            return taskA.category > taskB.category;
                        }));
                    });
                },
                ...
                saveTask: function(task, callback) {
                    db.transaction(function(transaction) {
                        ...
                        if (! task.id) {
                            transaction.executeSql(
                                "INSERT INTO task(name, description, due, priority, category) VALUES (?, ?, ?, ?, ?);", 
                                [task.name, task.description, task.due, task.priority, task.category],
                                function(tx) {
                                    transaction.executeSql(
                        ...
                        // otherwise, update
                            ...
                            transaction.executeSql(
                                "UPDATE task " +
                                "SET name = ?, description = ?, due = ?, completed = ?, priority = ?, category = ?" + 
                                "WHERE rowid = ?;",
                                [task.name, task.description, task.due, task.completed, task.priority, task.category, task.id],
                                function (tx) {
                                    if (callback) {
                                        callback();
                                    } // if
        ...
        activateCategoryTasks: function() {
			TODOLIST.Storage.getAllCategories(function(categories) {
                // update the current tasks
                currentCategories = categories;
			});
            TODOLIST.Storage.getTasksInCategoryOrder(function(tasks) {
                // update the current tasks
                currentTasks = tasks;
                ...
                // refresh the task list display
                jQuery("ul#categorytasklist li").click(function() {
                    toggleDetailsDisplay(this);
                });

                jQuery("ul#categorytasklist a.task-delete").click(function() {
                    // delete the task
...
$(document).ready(function() {
    ...
    $("#categorytasks").bind("activated", TODOLIST.activateCategoryTasks);
	...
	myScroll2 = new iScroll('categorytaskscroller', {desktopCompatibility:true});
	myScroll3 = new iScroll('categoryscroller', {desktopCompatibility:true});
	var wrapperH = window.innerHeight - 100;
	document.getElementById('wrapperalltasks').style.height = wrapperH;
	document.getElementById('wrapperoldtasks').style.height = wrapperH;
	document.getElementById('wrappercategorytasks').style.height = wrapperH;
	document.getElementById('wrappercategories').style.height = wrapperH;
